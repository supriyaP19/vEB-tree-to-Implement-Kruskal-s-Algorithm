...
void Heap::decreaseKey(Node_Fibonacci *n, int newKey)
{
    // Precondition: newKey < n -> key
    n -> key = newKey;
    
    Node_Fibonacci *current_node = n;
    if (current_node -> parent != NULL)
    {
        if (current_node -> key < current_node -> parent -> key)
        {
            Node_Fibonacci *parent = current_node -> parent;
            current_node -> marked = false;
            
            current_node -> parent = NULL;
            if (current_node -> forward == current_node) parent -> child = NULL;
            else
            {
                Node_Fibonacci *prev = current_node -> backward;
                Node_Fibonacci *next = current_node -> forward;
                prev -> forward = next;
                next -> backward = prev;
                if (parent -> child == current_node) parent -> child = prev;
            }
            parent -> degree = parent -> degree - 1;
            
            Node_Fibonacci *last = this -> min -> backward;
            last -> forward = current_node;
            current_node -> backward = last;
            this -> min -> backward = current_node;
            current_node -> forward = this -> min;
            
            if (current_node -> key < this -> min -> key) this -> min = current_node;
            ...
            if (parent -> parent != NULL) parent -> marked = true;
        }
    }
    else if (n -> key < this -> min -> key) this -> min = n;
}

void Heap::Delete(Node_Fibonacci *n)
{
...
int main()
{
    ...
    // Node_Fibonacci *x = new Node_Fibonacci(11);
    // Node_Fibonacci *y = new Node_Fibonacci(5);
    
    // fh -> insert(x);
    // fh -> insert(y);
    // fh -> insert(new Node_Fibonacci(3));
    // fh -> insert(new Node_Fibonacci(8));
    // fh -> insert(new Node_Fibonacci(4));
    // fh -> insert(new Node_Fibonacci(12));
    // fh -> insert(new Node_Fibonacci(1));
    ...
    //struct edges e[g_edges+1];
    for(int i=0;i<g_edges;i++){
        
        // e[i].from=g_from[i];
        // e[i].to=g_to[i];
        // e[i].weight=g_weight[i];
